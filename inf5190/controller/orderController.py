...
class OrderController:
    ...
    def create_order(cls, post_data):
        ...
        order_r = 0
        i = 0
        for rows in post_data["products"]:

            # "product" in post_data and "id" in rows.post_data["product"] and "quantity" in rows.post_data["product"] \
            # and isinstance(rows.post_data["product"]["id"], int) and \
            # isinstance(rows.post_data["product"]["quantity"], int) \
            # and rows.post_data["product"]["quantity"] > 0:
            product_id = rows["id"]
            ...
            if i == 0:
                ...
                order = Order.update(product_id=product_id, quantity=quantity,
                                     total_price=Order.total_price + total_price,
                                     shipping_price=Order.shipping_price + shipping_price, paid=False)
                total_price = Order.total_price + total_price
                shipping_price = Order.shipping_price + shipping_price

        return views.display_post_redirect(order_r)
